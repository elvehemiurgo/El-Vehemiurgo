---
titulo: "Guion 003 — Déjenlo ser (2/3): Royce Keys, el mismo error con otro nombre"
tipo: guion-editorial
numero: 003
fecha: 2026-09-04
duracion_objetivo: "30 segundos"
serie: "Déjenlo ser — 3 partes"
formato_de_show: "YYYY MM DD Nombre del Show (ley del Vehemiurgo, 2026-09-04)"
fuente_cruda:
  - "notebook/2026-08-01-s61-nxt-heatwave-300826-cont-raw-310826-nxt-010926.md (2026 08 31 WWE Raw)"
doctrina: "archive/topics/doctrina-forzar-al-babyface-a-dar-lastima.md"
estado: draft
ultima_actualizacion: 2026-09-04
tags: [guion, dejenlo-ser, royce-keys, jacob-fatu, roman-reigns, otm, wwe-raw-2026, doctrina-forzar-lastima]
---

# Guion 003 — Déjenlo ser (2/3): Royce Keys

> **Regla de estilo (Vehemiurgo, 2026-07-14)**: el texto base es su
> opinión TAL CUAL; la data del copiloto va **en cursiva y entre
> paréntesis**, recortable sin que el texto se caiga.
>
> **Formato de show (Vehemiurgo, 2026-09-04)**: todo show se nombra
> **`YYYY MM DD Nombre del Show`**.
>
> **Serie de tres**. Este es el caso **en curso**: el error repetido,
> con la corrección ya empezando a operar en pantalla.

## 1. La opinión cruda, corregida

*(Verbatim del Vehemiurgo, 2026-09-04 s61, sobre **2026 08 31 WWE
Raw** — solo gramática y puntuación.)*

> Me gusta que armen lo de Royce Keys y Jacob como una grudge match
> entre ex amigos, apegando el gimmick de Royce a un origen violento
> junto con Jacob. Incluso Jacob lo puso over diciendo que Royce es el
> verdadero problema.
>
> Es una historia mucho más interesante y entretenida que Royce dando
> lástima porque de niño era pobre. Fue un fracaso tanto promo video,
> igual que con Montana: fracasaron forzándolo a dar lástima como niño
> pobre de Nueva York, y Montana logró levantar su acto cuando lo
> dejaron ser y se puso a hablar como el face badass que en verdad es.
> Lo mismo podría pasar con Royce.

## 2. Memoria del archivo

- **2026 06 12 WWE SmackDown** — primer registro individual de Royce
  Keys en el archivo (four-way con Jey Uso, LA Knight y Finn Bálor).
  **Clase: WE.**
  [`../archive/people/royce-keys.md`](../archive/people/royce-keys.md)
- **2026 08 03 WWE Raw** — **Jacob Fatu lo destroza en un Street
  Fight** (moonsault + Tongan Death Grip), con el visto bueno de Roman
  Reigns. **Ese es el vínculo previo que ahora se reencuadra como
  "ex amigos".** *(Sin ficha de match propia — registrado solo en la
  trayectoria de Royce Keys; sin clase.)*
- **2026 08 24 WWE Raw (s50)** — se registra su pertenencia a **OTM**
  junto a Bronco Nima y Lucien Price, y queda anotado como pendiente
  el hilo con Jacob Fatu. **Clase del match (LA Knight & Solo Sikoa vs
  los Usos): FS·WE → Feeling Crown.**
- **2026 08 31 WWE Raw (s61)** — el Vehemiurgo cita **su propio
  precedente sobre Montana** para diagnosticar el mismo error. **Las
  tres piezas del show donde vive Royce Keys**: apertura OTM/Usos —
  **WE**; promo de Fatu en la mesa — **WE**; segmento de Roman Reigns —
  **sin clase** *(nota de booking, por decisión del Vehemiurgo)*. De ahí
  nace el topic
  [`../archive/topics/doctrina-forzar-al-babyface-a-dar-lastima.md`](../archive/topics/doctrina-forzar-al-babyface-a-dar-lastima.md).

## 3. La data real

| Dato | Fuente / estado |
|---|---|
| **2026 08 31 WWE Raw** — Rocket Arena, Cleveland, Ohio. **Grabado el 28/8**, no emitido en vivo. | Research s61 (2) |
| **Apertura del show**: OTM (Keys, Bronco Nima, Lucien Price) embosca a los Usos en el estacionamiento. **Keys rompe el parabrisas con una palanca y saca a Jey Uso sangrando**; Nima y Price chokeslamean a Jimmy contra el vidrio trasero. **Jacob Fatu llega con un caño y los tres huyen.** Los Usos quedan fuera de programación. | Research s61 (2), corroborado por 5+ outlets |
| **Segmento de Roman Reigns (midcard)**: Reigns **le ordena a Jacob Fatu "manejar el problema de Royce Keys"**; Fatu responde enmarcando a Keys como **amenaza mayor que él mismo**. | Video oficial WWE (1) + research (2) |
| **Un match Keys vs Fatu NO quedó anunciado formalmente** esa noche — solo la orden verbal. | `[no confirmado]` — **no afirmar en el guion** |
| El **vignette de origen** (infancia en East Palo Alto, California) existe en el catálogo de WWE, documentado como parte de su **debut en WWE SmackDown, abril de 2026** *(día exacto no confirmado)*. **No se confirma que se reemitiera el 31/8.** | `[no confirmado]` para el día y para la reemisión |

## 4. El guion afilado *(≈30 segundos)*

> Royce dando lástima porque de niño era pobre **fue un fracaso**.
> Tanto promo video *(el vignette es de abril, de su debut en WWE
> SmackDown)*.
>
> Igual que con Montana: fracasaron forzándolo a dar lástima como niño
> pobre de Nueva York.
>
> Ahora me gusta que armen lo de Royce y Jacob como **grudge match
> entre ex amigos**, apegando el gimmick de Royce a un **origen
> violento** *(2026 08 31 WWE Raw)*. Jacob lo puso over diciendo que
> **Royce es el verdadero problema**.
>
> Eso es mucho más interesante. Lo mismo podría pasar con Royce.

## 5. Timestamps y referencias en pantalla

| Tiempo | Línea | Show *(YYYY MM DD)* | Ubicación en el show | Qué se menciona | Clase de la pieza |
|---|---|---|---|---|---|
| **0:00–0:06** | *"Royce dando lástima porque de niño era pobre fue un fracaso"* | **2026 04 [día no confirmado] WWE SmackDown** | vignette de debut | Video de origen: infancia en East Palo Alto. **No confirmado que se reemitiera el 31/8** | *no es pieza registrada — sin clase* |
| **0:06–0:11** | *"Igual que con Montana… niño pobre de Nueva York"* | **2026 08 04 WWE NXT** → **2026 08 31 WWE Raw** | precedente cruzado | El caso Montana, ya resuelto (ver Guion 002) | **WE** *(25/8)* → **sin clase** *(31/8)* |
| **0:11–0:22** | *"Grudge match entre ex amigos, origen violento"* | **2026 08 31 WWE Raw** *(Rocket Arena, Cleveland — grabado el 28/8)* | **midcard**, segmento in-ring de Roman Reigns | Reigns le ordena a Fatu manejar a Keys; el reencuadre del gimmick | **sin clase** *(nota de booking)* |
| **0:22–0:30** | *"Jacob lo puso over: Royce es el verdadero problema"* | **2026 08 31 WWE Raw** | ídem, misma respuesta de Fatu a Reigns | Fatu enmarca a Keys como amenaza mayor que él mismo | **sin clase** *(mismo segmento)* |

**Material de apoyo sugerido (B-roll)**:

- **2026 08 31 WWE Raw** — **apertura del show**: OTM emboscando a los
  Usos en el estacionamiento, Keys rompiendo el parabrisas con la
  palanca y sacando a Jey sangrando. **WE.**
- **2026 08 31 WWE Raw** — **inmediatamente después**: Jacob Fatu
  sobre la mesa de comentaristas declarándole la guerra a Keys.
  **WE.**
- **2026 08 03 WWE Raw** — el Street Fight en que Fatu destroza a
  Keys: el pasado que ahora se convierte en *"ex amigos"*. *(Sin ficha
  de match; sin clase.)*

> **Cuidado en edición**: **no** afirmar que la lucha Keys vs Fatu está
> anunciada. Al **2026 08 31 WWE Raw** solo existe la orden verbal de
> Reigns.

## 6. Cierre de serie

Sigue en **[Guion 004](./guion-004-dejenlo-ser-joe-hendry.md)** — la
misma receta, pero dicha **antes** de que la empresa se equivoque.
