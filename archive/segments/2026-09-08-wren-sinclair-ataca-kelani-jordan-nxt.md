---
segmento: "Wren Sinclair ataca a Kelani Jordan con un kendo stick — suspensión de Robert Stone"
slug: "2026-09-08-wren-sinclair-ataca-kelani-jordan-nxt"
tipo_segmento: "post-match angle"
protagonistas: ["Wren Sinclair", "Kelani Jordan", "Victoria D'Errico", "Robert Stone"]
empresa: "WWE"
programa: "NXT"
fecha: 2026-09-08
ciudad: "Orlando, Florida"
recinto: "WWE Performance Center"
ubicacion_en_show: "post-match, tras la apertura"
duracion: "[no confirmado]"
linea_textual: "[paráfrasis reportada] Sinclair: lo hizo porque Jordan lesionó a Kendal Grey y la llamó 'sidekick', no le dejó opción; ante la suspensión, acusa que la castigan justo cuando le creció la columna (Ringside News)"
gimmick_momento: "la vengadora golpeando por accidente a la autoridad que debía detenerla"
storyline: "Wren Sinclair venga el ataque de Kelani Jordan sobre Kendal Grey (a quien llamó 'sidekick'), interrumpiendo el festejo del título con un kendo stick. Golpea sin querer a la árbitra Victoria D'Errico al forcejear; Robert Stone la suspende indefinidamente después del show por agredir a una oficial."
attendance_anunciada: ""
attendance_pagada: ""
gate: ""
rating_tv: ""
veces_visto_vehemiurgo: 0
calificacion_vehemiurgo: ""
clases_vehemiurgo: []
estado: en-investigacion
ultima_actualizacion: 2026-09-23
fuentes_principales:
  - "Sub-agente research-wwe-nxt-080926 (research 2026-09-12) — WebSearch (Cageside Seats, Fightful, SEScoops, Wrestling Attitude); WebFetch bloqueado por egress"
  - "Sub-agente pendientes-fichas-sep26 (research 2026-09-23) — WebSearch (Ringside News, Wrestlezone, SEScoops); WebFetch bloqueado por egress"
tags: [wwe-nxt-2026, wren-sinclair, kelani-jordan, kendal-grey, robert-stone, victoria-derrico, suspension, kendo-stick]
---

# Wren Sinclair ataca a Kelani Jordan — suspensión de Robert Stone (8/9/2026)

> **Pieza cerrada por research, no por dictado directo** — el
> Vehemiurgo mencionó a Wren Sinclair en el timestamp 00:15:07 sin
> nombrar rival ni contexto; el research (`wwe-nxt-080926`) identificó
> que se trataba de este ataque post-match, no de un match propio.

## Resumen

Tras retener Kelani Jordan el NXT Women's Championship ante Jaida
Parker, **Wren Sinclair** irrumpe con un **kendo stick** sacado de
debajo del ring y castiga a Jordan sin piedad hasta que intervienen
oficiales. En el forcejeo **golpea por accidente a la árbitra
Victoria D'Errico**. Post-show, **Robert Stone la suspende
indefinidamente** por agredir a una oficial.

## Qué pasó (descripción del segmento)

- Sinclair entra al ring y ataca a Jordan apenas termina la
  celebración del título.
- **Motivo declarado**: venganza porque Jordan lesionó a **Kendal
  Grey** y la llamó *"sidekick"* (subordinada).
- Golpea accidentalmente a la referí D'Errico al intentar que
  rompiera el ataque.
- Saca un kendo stick de debajo del ring y sigue el castigo hasta la
  intervención de oficiales.
- **Consecuencia (reportada post-show, no en pantalla esa noche)**:
  Robert Stone la suspende indefinidamente.

## Lectura del Vehemiurgo

- **Por qué está en mi base**: mencionada por el Vehemiurgo sin
  contexto completo — el archivo cierra el hueco con research, según
  la ley de sub-agentes obligatorios (fecha + show conocidos).
- **Veces que lo vi**: sin dato — el Vehemiurgo no declaró opinión
  sobre este segmento específico, solo mencionó a Sinclair de forma
  genérica en la lucha anterior ("otra que está destacando... la veo
  lista para historias con más stakes").
- **Clase**: sin declarar.

## Pendientes

- [x] Ciudad/recinto → Orlando, Florida — WWE Performance Center
      (SEScoops). Línea → paráfrasis reportada en `linea_textual`
      (Ringside News).
- [ ] Línea textual verbatim y duración exacta — sin fuente.
- [x] Confirmar si la lesión de Kendal Grey a manos de Jordan está
      registrada en otra ficha del archivo. → Sí:
      `2026-09-01-promo-kelani-jordan-campeona-jaida-parker-nxt` (storyline:
      Jordan explota la lesión de oído/equilibrio en Heatwave; Grey fuera
      por vértigo), `2026-08-30-promo-video-kendal-grey-vs-kelani-jordan-nxt-heatwave`
      y `people/kendal-grey.md`.

## Cross-links

- [`../people/wren-sinclair.md`](../people/wren-sinclair.md)
- [`../people/kelani-jordan.md`](../people/kelani-jordan.md)
- [`../people/robert-stone.md`](../people/robert-stone.md)
- [`../matches/2026-09-08-jaida-parker-vs-kelani-jordan-nxt.md`](../matches/2026-09-08-jaida-parker-vs-kelani-jordan-nxt.md)
- Volcado: [`../../notebook/2026-09-12-s01-nxt-080926.md`](../../notebook/2026-09-12-s01-nxt-080926.md)
