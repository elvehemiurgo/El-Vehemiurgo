---
nombre: "Las Tóxicas"
tipo: fact-sheet
categoria: promotions
slug: las-toxicas
estado: vivo
ultima_actualizacion: 2026-07-14
fuentes_principales:
  - "Volcado Vehemiurgo 2026-07-14 s35 (visión directa — AAA Worldwide, 4/4/2026)"
  - "Mención previa en dossier AAA/AEW 2026 cluster (roster AAA Knockouts)"
tags: [las-toxicas, aaa, ko-division, primer-registro, declaracion-de-carino-editorial]
---

# Las Tóxicas

> Facción/campeonas de la división Knockouts de AAA. Primer
> registro con desarrollo propio del archivo — el Vehemiurgo
> declara que *"se ganaron mi corazón"*, una declaración de
> afecto fuerte que **no usa la fórmula del panteón** ("panteón de
> héroes") y por tanto no se ejecuta como alta al panteón.

## Identificación

- **Nombre**: Las Tóxicas.
- **Empresa (2026)**: AAA — división Knockouts.
- **Integrantes**: [verif — el roster de Knockouts AAA 2026 lista
  también a Adelicious, Fabi Apache, Jessy Jackson, La Hiedra,
  Lady Flamer, Maravilla; relación exacta de Las Tóxicas con Lady
  Flamer [verif]].

## Notas editoriales del Vehemiurgo

### Sesión 2026-07-14 s35 — "Se ganaron mi corazón" (sin alta al panteón)

**Cita verbatim**:

> *"está bueno el bookeo de las toxicas todo el año, y este
> reinado tiene buenas defensas [...] pongamos a las toxicas en
> luchadores que se ganaron mi corazón cada una por separado,
> definitivamente"*
>
> — El Vehemiurgo, 2026-07-14 s35 (verbatim)

**Lectura sintética**: elogio de booking sostenido durante todo el
año (reinado con buenas defensas) y una declaración de cariño
fuerte que **no invoca la fórmula operativa del panteón**
("panteón de héroes", "dios del wrestling", "héroe fundamental").
Se registra tal cual, como afecto editorial de alta intensidad,
sin ejecutar el protocolo de alta al panteón — distinción
deliberada para no fabricar una alta que el Vehemiurgo no declaró
con el vocabulario exacto. Si el Vehemiurgo confirma que quiso
decir panteón, se ejecuta el protocolo en la próxima sesión. Ver
[`../matches/2026-04-04-lady-flamer-vs-sussy-aaa-worldwide.md`](../matches/2026-04-04-lady-flamer-vs-sussy-aaa-worldwide.md).

## Pendientes / huecos

- [ ] Confirmar si "cada una por separado" implica alta al
      panteón o es una lista distinta — [verif], preguntar al
      Vehemiurgo.
- [ ] Roster exacto de Las Tóxicas como campeonas/facción.
- [ ] Cronología del reinado y sus defensas durante el año.

## Cross-links

- [`../people/lady-flamer.md`](../people/lady-flamer.md)
