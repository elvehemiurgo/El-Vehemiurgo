---
match: "Queen Aminata vs Mina Shirakawa"
slug: "2026-09-12-queen-aminata-vs-mina-shirakawa-aew-collision"
participantes: ["Queen Aminata", "Mina Shirakawa"]
empresa: "AEW"
programa: "Collision"
fecha: 2026-09-12
ciudad: "Springfield, Massachusetts"
recinto: "MassMutual Center"
tipo_match: "singles"
estipulacion: "standard"
duracion: "10:19 [una fuente]"
finish: "Shirakawa va por un cuarto intento de Figure Four; Aminata cuelga un small package/inside cradle y roba el pin"
ganador: "Queen Aminata"
referee: "[verif]"
encuentros_previos: 0
veces_visto_vehemiurgo: 1
calificacion_vehemiurgo: "Estuvo cool"
clases_vehemiurgo: ["fighting-spirit", "wrestling-entertainment"]
estado: en-investigacion
ultima_actualizacion: 2026-09-23
fuentes_principales:
  - "Volcado Vehemiurgo 2026-09-18 s01 (visión directa — AEW Collision, 12/9/2026, timestamp 00:36:49)"
  - "Sub-agente research-aew-cards-050926-090926-120926 (research 2026-09-18) — WebSearch (PWTorch, Wrestling Inc.); WebFetch bloqueado por egress"
  - "Sub-agente pendientes-fichas-sep26 (research 2026-09-23) — WebSearch (411mania); WebFetch bloqueado por egress"
tags: [aew-collision-2026, queen-aminata, mina-shirakawa, mercedes-mone, feeling-crown, andy-williams-tribute-show]
---

# Queen Aminata vs Mina Shirakawa (Collision, 12/9/2026)

> **Feeling Crown** (FS·WE). Con **Mercedes Moné en comentarios**.
> Show tributo a "The Butcher" Andy Williams (ver
> [`./2026-09-12-andy-williams-battle-royal-aew-collision.md`](./2026-09-12-andy-williams-battle-royal-aew-collision.md)
> para el contexto completo).

## Resumen

Timestamp **00:36:49**. Queen Aminata vence a Mina Shirakawa.
Shirakawa va por un cuarto intento de Figure Four; Aminata cuelga un
small package/inside cradle y roba el pin.

## Lectura del Vehemiurgo

- **Por qué está en mi base**: FC declarada.
- **Veces que lo vi**: 1.
- **Clase**: **Fighting Spirit + Wrestling Entertainment → Feeling
  Crown.**

## Cita verbatim del Vehemiurgo

> *"Aminata vs Shirakawa estuvo cool, se merece una WE y FS"*
>
> — El Vehemiurgo, 2026-09-18 s01 (verbatim)

## Pendientes

- [x] Duración → 10:19 [una fuente] (Thomas Hall, 411mania)
- [ ] Referee.

## Cross-links

- [`../people/queen-aminata.md`](../people/queen-aminata.md)
- [`../people/mercedes-mone.md`](../people/mercedes-mone.md)
- Volcado: [`../../notebook/2026-09-18-s01-dynamite-020926-cont-collision-050926-rebel-heart-090926-collision-120926.md`](../../notebook/2026-09-18-s01-dynamite-020926-cont-collision-050926-rebel-heart-090926-collision-120926.md)
