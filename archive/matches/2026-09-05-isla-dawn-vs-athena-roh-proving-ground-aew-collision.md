---
match: "Athena (c) vs Isla Dawn — ROH World Women's Championship Proving Ground Match"
slug: "2026-09-05-isla-dawn-vs-athena-roh-proving-ground-aew-collision"
participantes: ["Athena", "Isla Dawn"]
empresa: "AEW/ROH"
programa: "Collision"
fecha: 2026-09-05
ciudad: "Allen, Texas"
recinto: "Credit Union of Texas Event Center"
tipo_match: "singles — ROH World Women's Championship Proving Ground Match"
estipulacion: "ROH World Women's Championship — Proving Ground (si Dawn no gana, no recibe derecho a futura oportunidad titular)"
duracion: "[no confirmado]"
finish: "[no confirmado] — pinfall de Athena"
ganador: "Athena retiene el ROH World Women's Championship"
referee: "[verif]"
encuentros_previos: 0
veces_visto_vehemiurgo: 1
calificacion_vehemiurgo: "Fue genial, un throwback, hace tiempo no veía una lucha así de Isla Dawn; se pusieron stiff, muy oldschool"
clases_vehemiurgo: ["perfect-wrestling", "fighting-spirit", "wrestling-entertainment-plus"]
estado: en-investigacion
ultima_actualizacion: 2026-09-18
fuentes_principales:
  - "Volcado Vehemiurgo 2026-09-18 s01 (visión directa — AEW Collision, 5/9/2026, timestamp 01:24:04)"
  - "Sub-agente research-aew-cards-050926-090926-120926 (research 2026-09-18) — WebSearch (Slam Wrestling); WebFetch bloqueado por egress"
tags: [aew-collision-2026, athena, isla-dawn, roh-womens-world-championship, proving-ground, instant-classic-crown-plus, throwback, oldschool]
---

# Athena (c) vs Isla Dawn — ROH World Women's Championship (Collision, 5/9/2026)

> **Instant Classic Crown+** (PW·FS·WE+). *"Un throwback... se
> pusieron stiff, muy oldschool"* — **Proving Ground Match**: Athena
> retiene, y Dawn queda sin derecho a futura oportunidad titular por
> la estipulación.

## Resumen

Timestamp **01:24:04**. Athena retiene el ROH World Women's
Championship ante Isla Dawn en un Proving Ground Match.

## Lectura del Vehemiurgo

- **Por qué está en mi base**: **Instant Classic Crown+ declarada.**
- **Veces que lo vi**: 1.
- **Clase**: **Perfect Wrestling + Fighting Spirit + Wrestling
  Entertainment+ → Instant Classic Crown+.**

## Cita verbatim del Vehemiurgo

> *"Isla Dawn vs Athena, fue genial, un throwback, hjace tiempo no
> veia una lucha así de Isla Dawn, se merece buenos oponentes,
> siempre fue buena, en esta lucha se pusieron stiff, muy oldschool,
> se merecen las 3 clases con la corona +"*
>
> — El Vehemiurgo, 2026-09-18 s01 (verbatim, typos preservados)

**Lectura sintética**:

1. **"Un throwback"**: lectura de estilo — trabajo físico y creíble,
   coherente con la doctrina old-school del proyecto.
2. **Proving Ground Match, confirmado por research**: Dawn no gana
   derecho a futura oportunidad titular por perder esta estipulación
   — el "buenos oponentes" que el Vehemiurgo le desea puede no llegar
   pronto por esta vía específica.

## Pendientes

- [ ] Duración exacta, finish exacto, referee.

## Cross-links

- [`../people/athena.md`](../people/athena.md)
- [`../people/isla-dawn.md`](../people/isla-dawn.md)
- Volcado: [`../../notebook/2026-09-18-s01-dynamite-020926-cont-collision-050926-rebel-heart-090926-collision-120926.md`](../../notebook/2026-09-18-s01-dynamite-020926-cont-collision-050926-rebel-heart-090926-collision-120926.md)
