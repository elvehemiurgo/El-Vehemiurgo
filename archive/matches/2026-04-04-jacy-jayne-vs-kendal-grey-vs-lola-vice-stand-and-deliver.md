---
match: "Jacy Jayne vs Kendal Grey vs Lola Vice — triple threat"
slug: "jacy-jayne-vs-kendal-grey-vs-lola-vice-stand-and-deliver-2026-04-04"
participantes:
  - "Jacy Jayne (Fatal Influence)"
  - "Kendal Grey (WrenQCC member)"
  - "Lola Vice (Fatal Influence — defending NXT Women's Title)"
empresa: "WWE NXT"
programa: "Stand & Deliver 2026"
fecha: 2026-04-04
ciudad: "[verif]"
recinto: "[verif]"
tipo_match: "triple threat — NXT Women's Championship match"
estipulacion: "NXT Women's Championship"
duracion: ""
finish: ""
ganador: ""
referee: ""
attendance_anunciada: ""
attendance_pagada: ""
gate: ""
rating_tv: ""
buy_rate: ""
encuentros_previos: 1
veces_visto_vehemiurgo: 1
calificacion_vehemiurgo: ""
clases_vehemiurgo: []
estado: en-investigacion
ultima_actualizacion: 2026-05-10
fuentes_principales:
  - "Notebook del Vehemiurgo + Dossier NXT 2025-2026 Stand & Deliver callups"
tags: [nxt-2026, stand-and-deliver-2026, nxt-womens-title, fatal-influence, jacy-jayne, kendal-grey, lola-vice, wrenqcc, triple-threat-women, pre-callup-build]
---

# Jacy Jayne vs Kendal Grey vs Lola Vice — NXT Women's Title triple threat — Stand & Deliver (4 abr 2026)

> **Triple threat con Fatal Influence intra-faction
> dynamic** + outsider Kendal Grey (WrenQCC member, ex-Evolve
> Women's Champion). Pieza centerpiece del cluster en
> [`../../dossiers/draft-nxt-2025-2026-stand-deliver-callups.md`](../../dossiers/draft-nxt-2025-2026-stand-deliver-callups.md).

## Storyline y construcción

- **Fatal Influence** stable consolidada con Lola Vice como
  NXT Women's Champion + Jacy Jayne como heir-apparent.
- **Kendal Grey** post-Evolve Women's Title period + WrenQCC
  member (formación 7 abr 2026 — esta match es 3 días previa).
- **17 mar 2026 NXT**: Jacy Jayne vs Sol Ruca vs Saria
  (Saria-Sol arc paralelo).
- **31 mar 2026 NXT**: Lola Vice vs Kendal Grey.
- **4 abr 2026 Stand & Deliver** *(este match)*: triple threat.

## Significancia editorial

- **Intra-faction dynamic**: dos Fatal Influence members
  (Jacy + Lola) + outsider (Kendal Grey) → genera tensión
  storyline propia. ¿Lola retiene? ¿Jacy traiciona? ¿Kendal
  pulls upset? **3 escenarios narrativos viables**.
- **Pre-SmackDown call-up**: el match es **3 semanas antes
  del Fatal Influence debut SmackDown 24 abr 2026** (✓ Batch 1).
  Las decisiones del booking acá definen **quién entra a main
  roster con título y quién no**.
- **WrenQCC member presence**: Kendal en el match valida
  WrenQCC tag como **storyline activa** en title pictures
  individuales. Cross-link con
  [`../promotions/wrenqcc.md`](../promotions/wrenqcc.md).

## Lectura del Vehemiurgo

### Sesión 2026-07-14 s35 — Elaboración cortada, sin clase registrada

**Cita verbatim (mensaje cortado)**:

> *"la 3 way de Grey vs Vice vs Jane estuvo muy cool si se merece
> el"* [mensaje interrumpido — el Vehemiurgo no completó la
> declaración de clase]
>
> — El Vehemiurgo, 2026-07-14 s35 (verbatim; "Jane" = variante de
> dictado de Jacy Jayne)

**Lectura sintética**: elaboración sobre este match ya existente
en el archivo — se registra únicamente lo dicho ("estuvo muy
cool"), **sin asignar clase**, porque la oración quedó incompleta
antes de nombrar la fórmula. Si el Vehemiurgo retoma la idea en
una sesión futura, se completa acá mismo.

## Pendientes / huecos

- [ ] Ganador + finish + duración.
- [ ] Star rating WON Meltzer (contexto).
- [ ] **Asignación de clase del Vehemiurgo** — pendiente:
      declaración cortada en s35 (2026-07-14), a completar cuando
      el Vehemiurgo retome la idea.

## Piezas relacionadas

- [`../../dossiers/draft-nxt-2025-2026-stand-deliver-callups.md`](../../dossiers/draft-nxt-2025-2026-stand-deliver-callups.md).
- [`../promotions/fatal-influence.md`](../promotions/fatal-influence.md).
- [`../promotions/wrenqcc.md`](../promotions/wrenqcc.md).
- [`../segments/2026-04-24-fatal-influence-debut-smackdown.md`](../segments/2026-04-24-fatal-influence-debut-smackdown.md)
  — call-up payoff.
