---
match: "Mustafa Ali & Tasha Steelz vs Trey Miguel & Jada Stone — mixed tag [verif configuración]"
slug: "2026-03-27-ali-tasha-vs-trey-jada-stone-mixed-tag-tna-sacrifice"
participantes: ["Mustafa Ali", "Tasha Steelz", "Trey Miguel", "Jada Stone"]
empresa: "TNA Wrestling"
programa: "Sacrifice 2026"
fecha: 2026-03-27
ciudad: "[verif]"
recinto: "[verif]"
tipo_match: "mixed tag"
estipulacion: "standard"
duracion: "[verif]"
finish: "[verif]"
ganador: "[verif]"
referee: "[verif]"
encuentros_previos: 0
veces_visto_vehemiurgo: 1
calificacion_vehemiurgo: "FS explícita + WE ratificada explícitamente el 2026-07-09 ('world class' → Wrestling Entertainment: 'ratifico WE')"
clases_vehemiurgo: ["fighting-spirit", "wrestling-entertainment"]
estado: stub
ultima_actualizacion: 2026-07-09
fuentes_principales:
  - "Volcado Vehemiurgo 2026-07-05 s02 (visión directa — TNA Sacrifice)"
  - "Ratificación WE: Vehemiurgo 2026-07-09 s01"
tags: [mustafa-ali, tasha-steelz, trey-miguel, jada-stone, tna-sacrifice-2026, mixed-tag, fighting-spirit, wrestling-entertainment, we-ratificada-2026-07-09, ali-corrige-el-tag-oldschool, jada-stone-revelacion, quimica-ali-trey]
---

# Ali & Tasha Steelz vs Trey Miguel & Jada Stone — TNA Sacrifice (27 mar 2026)

> **FS explícita + WE ratificada** (*"la tag team en Sacrifice se
> merece fighting spirit y world class"* — "world class" se
> interpretó como Wrestling Entertainment y el Vehemiurgo la
> **ratificó explícitamente el 2026-07-09**). El match del
> **spot doctrinal**: Ali corrige el tag de Tasha en pleno match —
> *"hacer respetar lo oldschool y las reglas"*.

## Cita verbatim del Vehemiurgo

> *"con Trey Miguel tiene muy buena química; además del showcase
> para Jaida Stone como performer revelación, trabajando muy duro,
> vendiendo un gran acto babyface, y Tasha que es efectiva, y en
> esta lucha estuvo en un spot donde Ali le corrige el tag, y es
> genial ver a un luchador así hacer respetar lo oldschool y las
> reglas, ese debería ser el modelo para los nombres nuevos; la
> tag team en Sacrifice se merece fighting spirit y world class"*
>
> — El Vehemiurgo, 2026-07-05 s02 (verbatim; canónico Jada Stone)

**Lectura sintética**:

1. **El spot del tag corregido** — Ali (heel veterano) hace cumplir
   la regla del tag en vivo. **Doctrina**: el respeto a las reglas
   ES parte del acto — refuerza la realidad del mundo del match
   (mismo eje que el irish whip correcto, sesión #14). *"Ese
   debería ser el modelo para los nombres nuevos"* → pieza al
   horno: *"el veterano como enforcer de las reglas"*.
2. **Jada Stone showcase** — *"performer revelación"*, gran acto
   babyface, trabajo duro. THE FUTURE: **descartada** el
   2026-07-09 (*"muy indie [...] no creo que destaque mucho"*).
3. **Química Ali-Trey** — programa singles latente.
4. **Configuración [verif]** — equipos inferidos del take (Ali
   corrige el tag DE Tasha → mismo equipo).

## Pendientes

- [ ] Configuración + finish + duración + título en juego si lo hay.
- [x] **Ratificación WE** ("world class") — ratificada 2026-07-09.
- [x] ¿Jada Stone a THE FUTURE in 2026? — **no entra** (2026-07-09).

## Cross-links

- [`../people/mustafa-ali.md`](../people/mustafa-ali.md) ·
  [`../people/tasha-steelz.md`](../people/tasha-steelz.md) ·
  [`../people/trey-miguel.md`](../people/trey-miguel.md) ·
  [`../people/jada-stone.md`](../people/jada-stone.md)
- [`../topics/tna-2026-arbol-de-aprendizaje-en-mic.md`](../topics/tna-2026-arbol-de-aprendizaje-en-mic.md)
  — Tasha (Rama 1) y el enforcement de Ali como pedagogía en vivo.
