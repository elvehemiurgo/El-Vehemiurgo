---
nombre: "Chad Gable"
tipo: fact-sheet
categoria: people
slug: chad-gable
estado: vivo
ultima_actualizacion: 2026-08-26
fuentes_principales:
  - "Volcado Vehemiurgo 2026-08-01 s42 (visión directa — WWE Raw, 17/8/2026)"
  - "Sub-agente research-wwe-raw-170826-parte2 (research 2026-08-01, closed) — Wrestling Inc, PWTorch"
tags: [chad-gable, wwe-raw-2026, intercontinental-championship, rey-mysterio, fighting-spirit, wrestling-entertainment, primer-registro-individual]
---

# Chad Gable

> Primer registro individual del archivo. **WWE Intercontinental
> Champion** desde que le ganó el título a **Penta**; retiene su
> primera defensa contra **Rey Mysterio** en Raw.

## Identificación

- **Nombre de gimmick**: Chad Gable.
- **Nombre real**: [verif].
- **Empresa (agosto 2026)**: WWE Raw.
- **Título**: WWE Intercontinental Champion.

## Trayectoria condensada

- **10/8/2026 (Raw)** — recién coronado tras vencer a Penta, le
  ofrece la lucha titular a Rey Mysterio.
- **17/8/2026 (Raw, Buffalo)** — vs Rey Mysterio, primera defensa:
  **FS·WE**, retiene con victory roll tras intercambio de ankle
  lock/619s.
  ([→](../matches/2026-08-17-chad-gable-vs-rey-mysterio-ic-title-wwe-raw.md)).

## Notas editoriales del Vehemiurgo

### Sesión 2026-08-01 s42 — Primera defensa vs Mysterio: "gran lucha, pero muy safe"

**Cita verbatim**:

> *"Gable vs Mysterio fue una gran lucha, pero muy safe, muy lento,
> tiene buenos momentos, pero no es el clásico que pensarias, es un
> FS y WE classes. Está muy bien por ratos, pero en general
> tranquila."*
>
> — El Vehemiurgo, 2026-08-01 s42 (verbatim)

**Lectura sintética**: **FS·WE**, con reserva de ritmo sostenida en
todo el match, no puntual. Retiene el Intercontinental Championship.
Post-match (dato de research): recibe el gesto de respeto de Mysterio,
que le pide colocarse el cinturón él mismo — declara después que el
match fue *"everything I love about pro wrestling"*.

### Sesión 2026-08-26 s50 — El campeón que se queda con la última palabra (Raw 24/8/2026)

**Cita verbatim**:

> *"Luego el segmento backstage con Ethan y Gable estuvo cool, buenas
> promos, se ve bien el booking para Ethan como amenzaza real, incluso
> Gable es vio muy cool cerrando el segmento, se merece una WE class"*
>
> — El Vehemiurgo, 2026-08-26 s50 (verbatim)

**Lectura sintética**:

- **WE class declarada** para el segmento.
- **El *"incluso"* es el punto**: el campeón **cierra el segmento** y
  sale reforzado en el mismo promo donde se construye al retador.
  Doble ganancia, que es exactamente lo que un segmento de armado
  debería producir y rara vez produce.
- Contexto: **WWE Intercontinental Champion**, retuvo ante Rey
  Mysterio el 17/8/2026; ya había cruzado con Ethan Page en el
  gauntlet del 13/7.
- Ver [`../segments/2026-08-24-ethan-page-chad-gable-backstage-wwe-raw.md`](../segments/2026-08-24-ethan-page-chad-gable-backstage-wwe-raw.md).

## Pendientes / huecos

- [ ] Nombre real, debut, trayectoria completa.
- [ ] Fecha y contexto exacto de cuándo le ganó el título a Penta.

## Piezas del Vehemiurgo donde aparece

- [`../matches/2026-08-17-chad-gable-vs-rey-mysterio-ic-title-wwe-raw.md`](../matches/2026-08-17-chad-gable-vs-rey-mysterio-ic-title-wwe-raw.md)

## Cross-links

- [`./rey-mysterio.md`](./rey-mysterio.md) — rival, cierre de
  rivalidad sin rencor.
