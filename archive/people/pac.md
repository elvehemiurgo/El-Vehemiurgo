---
nombre: "PAC"
tipo: fact-sheet
categoria: people
slug: pac
estado: vivo
ultima_actualizacion: 2026-07-14
fuentes_principales:
  - "Volcado Vehemiurgo 2026-07-14 s14 (visión directa — AEW Collision Fairway to Hell, 9/5/2026)"
tags: [pac, aew, main-event-fairway-to-hell, muy-real, rival-panteon-darby]
---

# PAC

> Primer registro individual del archivo. Rival del main event
> *"muy real"* de Darby Allin en AEW Collision (Fairway to Hell,
> 9/5/2026) — Perfect Match que contribuyó a la alta de Darby al
> panteón (Héroe Fundamental #46).

## Identificación

- **Nombre de gimmick**: PAC.
- **Nombre real**: [verif].
- **Origen**: Reino Unido [verif].
- **Empresa actual (2026)**: AEW.

## Trayectoria condensada

- **9/5/2026 (AEW Collision, Fairway to Hell)** — main event vs
  Darby Allin: **Perfect Match** (PW·FS·WE).
  ([→](../matches/2026-05-09-darby-allin-vs-pac-aew-collision-fairway-to-hell.md)).

## Notas editoriales del Vehemiurgo

### Sesión 2026-07-14 s14 — Perfect Match "muy real" vs Darby

**Cita verbatim**:

> *"el main event de darby es genial, contra PAC, fue muy real, se
> merece todas las clases"*
>
> — El Vehemiurgo, 2026-07-14 s14 (verbatim)

**Lectura sintética**: entra al archivo con triple clase directa,
en el mismo eje de autenticidad ("muy real") que ya viene
marcando el Vehemiurgo en el tramo reciente de Darby Allin.

### Sesión 2026-07-14 s19 — Referencia de riesgo para Persephone

**Cita verbatim**:

> *"espero que refresquen la fórmula con ella, si no va a terminar
> como PAC que tiene el mismo booking desde que entró a AEW y solo
> gana cuando es sidekick de alguien"*
>
> — El Vehemiurgo, 2026-07-14 s19 (verbatim)

**Lectura sintética**: PAC se convierte en el ejemplo de referencia
de un patrón de booking estancado — "heel/talent confiado que no
gana luchas importantes por su cuenta" — usado como advertencia
para el desarrollo de Persephone. Ver
[`../segments/2026-05-30-persephone-post-match-aew-collision.md`](../segments/2026-05-30-persephone-post-match-aew-collision.md).

## Pendientes / huecos

- [ ] Nombre real, debut, trayectoria completa (PWG/NJPW/WWE
      NXT UK/AEW).
- [ ] Finish/duración del match.

## Piezas del Vehemiurgo donde aparece

- [`../matches/2026-05-09-darby-allin-vs-pac-aew-collision-fairway-to-hell.md`](../matches/2026-05-09-darby-allin-vs-pac-aew-collision-fairway-to-hell.md)

## Cross-links

- [`./darby-allin.md`](./darby-allin.md) — rival del Perfect
  Match.
