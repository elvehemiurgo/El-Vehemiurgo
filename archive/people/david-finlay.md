---
nombre: "David Finlay"
tipo: fact-sheet
categoria: people
slug: david-finlay
estado: vivo
ultima_actualizacion: 2026-07-14
fuentes_principales:
  - "Volcado Vehemiurgo 2026-07-14 s46 (visión directa — AEW Redemption, 26/7/2026)"
  - "Mención previa extensa en archive/people/the-dogs.md (s19-s34)"
tags: [david-finlay, the-dogs, bang-bang-gang, njpw-crossover, bullet-club-nostalgia, aew, primer-registro-individual]
---

# David Finlay

> Primer registro individual con ficha propia — hasta ahora
> cubierto solo dentro de la ficha de equipo
> [`the-dogs.md`](./the-dogs.md), donde el Vehemiurgo ya lo había
> declarado *"de verdad es una estrella"* (s26). Golpea con la
> cadena en la dog collar de Redemption, escalando formalmente su
> rivalidad con Jay White.

## Identificación

- **Nombre de gimmick**: David Finlay.
- **Nombre real**: [verif].
- **Equipo**: The Dogs / Bang Bang Gang.
- **Empresa (2026)**: AEW, con cruce NJPW.

## Trayectoria condensada

- Historial completo previo (rivalidad con Cope & Christian Cage,
  Guns vs Dogs, promo "histórica") documentado en
  [`the-dogs.md`](./the-dogs.md).
- **26/7/2026 (AEW Redemption)** — con Clark Connors vs Jay White
  & Juice Robinson (dog collar match): Wrestling Entertainment,
  con crítica de timing de estipulación.
  ([→](../matches/2026-07-26-jay-juice-vs-finlay-connors-dog-collar-aew-redemption.md)).

## Notas editoriales del Vehemiurgo

### Sesión 2026-07-14 s46 — El golpe con la cadena escala la rivalidad

**Cita verbatim**:

> *"el golpe con el chelele de finlay fue lo que escaló la
> rivalidad y ahora tenemos esto, que no me funciona como payoff
> pero [...] juice y jay merecen una dog collar muy bien bookeada
> con peso real de revancha; pero bueno la lucha está ok, se merece
> un enterteinment class"*
>
> — El Vehemiurgo, 2026-07-14 s46 (verbatim)

**Lectura sintética**: Finlay como el disparador físico de toda la
escalada (el golpe con la cadena) — el Vehemiurgo lee que ese
momento debía ser el detonante de una futura dog collar bien
bookeada, no algo resuelto en el mismo capítulo que lo generó. Ver
[`../matches/2026-07-26-jay-juice-vs-finlay-connors-dog-collar-aew-redemption.md`](../matches/2026-07-26-jay-juice-vs-finlay-connors-dog-collar-aew-redemption.md).

## Pendientes / huecos

- [ ] Nombre real, debut, trayectoria completa (NJPW, Bullet
      Club/Bang Bang Gang, AEW).
- [ ] **Prioridad**: aclarar si "The Dogs" (con Connors hoy) y "The
      Guns" tienen composición estable o si hay reconfiguración de
      equipos — contradicción con `clark-connors.md`, que lo
      tenía tentativamente ligado a "The Guns".
- [ ] Migrar el historial extenso ya documentado en
      `the-dogs.md` a expediente individual cuando el Vehemiurgo
      lo pida.

## Piezas del Vehemiurgo donde aparece

- [`../matches/2026-07-26-jay-juice-vs-finlay-connors-dog-collar-aew-redemption.md`](../matches/2026-07-26-jay-juice-vs-finlay-connors-dog-collar-aew-redemption.md)
- (Historial extenso previo: ver
  [`the-dogs.md`](./the-dogs.md).)

## Cross-links

- [`./the-dogs.md`](./the-dogs.md) — ficha de equipo.
- [`./clark-connors.md`](./clark-connors.md) — compañero en la dog
  collar match.
- [`./jay-white.md`](./jay-white.md) — rival.
