---
nombre: "Adam Priest"
tipo: fact-sheet
categoria: people
slug: adam-priest
estado: vivo
ultima_actualizacion: 2026-07-14
fuentes_principales:
  - "Volcado Vehemiurgo 2026-07-14 s26 (visión directa — AEW Collision, 20/6/2026)"
  - "Mención previa en la lista personal maestra"
tags: [adam-priest, aew, fighting-spirit, oldschool, quiere-luchar, primer-registro-individual]
---

# Adam Priest

> Primer take editorial individual (ya mencionado de pasada en la
> lista personal maestra). Debuta con **Fighting Spirit** contra
> Zack Sabre Jr: *"Priest sí quiere luchar"*.

## Identificación

- **Nombre de gimmick**: Adam Priest (identidad presumida — el
  Vehemiurgo dictó solo "Priest").
- **Nombre real**: [verif].
- **Empresa actual (2026)**: AEW.

## Trayectoria condensada

- **20/6/2026 (AEW Collision)** — vs Zack Sabre Jr: **Fighting
  Spirit**.
  ([→](../matches/2026-06-20-zsj-vs-adam-priest-aew-collision.md)).

## Notas editoriales del Vehemiurgo

### Sesión 2026-07-14 s26 — "Sí quiere luchar"

**Cita verbatim**:

> *"ZSJ vs Priest estuvo buena, Priest sí quiere luchar, y aquí le
> metieron muy bien, se merece el fighting spirit, estuvo muy
> oldschool"*
>
> — El Vehemiurgo, 2026-07-14 s26 (verbatim)

**Lectura sintética**: primera valoración — intención de combate
real leída como el activo principal.

## Pendientes / huecos

- [ ] [verif] confirmar identidad exacta ("Priest" del dictado =
      Adam Priest de la lista personal).
- [ ] Nombre real, debut, trayectoria.
- [ ] Finish/duración del match vs ZSJ.

## Piezas del Vehemiurgo donde aparece

- [`../matches/2026-06-20-zsj-vs-adam-priest-aew-collision.md`](../matches/2026-06-20-zsj-vs-adam-priest-aew-collision.md)

## Cross-links

- [`./zack-sabre-jr.md`](./zack-sabre-jr.md)
